...
void			my_draw(void)
{
	jo_printf(0, 0,"score=%d (c) 2017 Yeray Romano.",score,"fefa",niveles[0].visible);
	if (pausa != 1) {
	...
	while(cont < bolas || cont < migos || cont < 26) {
		if(cont < bolas) {
				...
				if(bolitas [cont].x >= 180 || bolitas [cont].x  <= -180) {
					locka = 1;
				...
				if(pausa == 0) {
					if(bolitas[cont].primigenio == 1) {
						...
						bolitas[cont].x += (2 * bolitas [cont].vector);
					}
				}
...
void			my_gamepad(void)
{
	...
	if(jo_is_pad1_key_pressed(JO_KEY_C) && pausa == 0 && vivo == 1) {
